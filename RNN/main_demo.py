...
import RNN.interprettensor.examples.recurrent as recurrent
from RNN.sub_gui.test_result import Ui_Form

class Ui_rnnWindow(QWidget):
    # CNN component button event functions
    def train(self):
        QtWidgets.QMessageBox.about(None, "Train message", "RNN 학습을 시작합니다.")
        self.textBrowser_3.setText("RNN 학습을 시작합니다.\n")
        try:
            self.th_train.start()
            QtWidgets.QMessageBox.about(None, "Train message", "RNN 학습 하는중...")
        except:
            QtWidgets.QMessageBox.about(None, "Error message", "에러발생:( 모델을 확인하세요.")
            self.textBrowser_2.setText("에러발생:( 모델을 확인하세요.")

    def test(self):
        QtWidgets.QMessageBox.about(None, "Test message", "RNN 테스트를 진행합니다.")
        self.textBrowser_3.setText("RNN 테스트를 진행합니다.\n")
        self.th_test.start()
    ...
    def showResult(self,msg):
        ...
        self.textBrowser_3.setText("RNN 테스트 결과 입니다.")
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "RNN main"))
        self.groupBox_4.setTitle(_translate("MainWindow", "RNN demo"))
    ...
    class Thread(QtCore.QThread):
        ...
        def run(self):
            if self.tag == 'train':
                recurrent.train(worker=self)
            elif  self.tag == 'test':
                recurrent.test(worker=self)
"""
